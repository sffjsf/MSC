...
def test_model(model, test_sets, config, epoch=None, saver=None):
    ...
    for set_name, test_set in test_sets.items():
        ...
        for j in range(titer):
            ...
            # Y['bdy'] = nn.functional.interpolate(Y['bdy'], size=out_shape, mode='bilinear')
            # Y1['final']=nn.functional.interpolate(Y1['final'], size=out_shape, mode='bilinear')
            pred = Y['final'].sigmoid_().cpu().data.numpy()[0, 0]
            # pred1 = Y['bdy'].cpu().data.numpy()[0, 0]
            # pred2 = Y1['final'].sigmoid_().cpu().data.numpy()[0, 0]

            pred, gt = normalize_pil(pred, gt)
            # pred1, gt = normalize_pil(pred1, gt)
