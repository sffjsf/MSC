...
def bce_ssim_loss(pred, gt):
    ...
    # mse=criterion2(pred, target)
    loss =  0.5*ssim_out.mean()+0.5* iou_out
    return loss
...
def Loss(preds, target, config):
    ...
    # preds['bdy'] = nn.functional.interpolate(preds['bdy'], size=target.size()[-2:], mode='bilinear')
    # loss += bce_ssim_loss(preds['bdy'], target)
...
# def Loss(preds, target1,target2, config):
#     loss = 0
#     ws = [1, 1]
#     a=[preds['final'],preds['final1']]
#     for pred, w,target in zip(a, ws,[target1,target2]):
#         pred = nn.functional.interpolate(pred, size=target.size()[-2:], mode='bilinear')
#         loss += bce_ssim_loss(torch.sigmoid(pred), target) * w
    
#     # loss+=consistency_loss(preds['final'],preds['final1'])/7
